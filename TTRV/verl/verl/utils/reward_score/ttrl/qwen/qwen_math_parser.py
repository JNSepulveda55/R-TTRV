...
def extract_answer(pred_str, data_name, use_last_number=True, option_number=None):
    # pred_str = pred_str.replace("\u043a\u0438", "")
    # print(f"Type of pred_str: {type(pred_str)}")
    # print(f"Value of pred_str: {pred_str}")
    if isinstance(pred_str, tuple):
        # If it's a tuple, take the first element (or loop and find the string)
        for item in pred_str:
            if isinstance(item, str):
                pred_str = item
                break
        else:
            pred_str = str(pred_str)  # fallback to string representation

    elif isinstance(pred_str, (dict, list)):
        # If it's a JSON-like structure, convert to string
        pred_str = json.dumps(pred_str)

    elif not isinstance(pred_str, str):
        # Any other type (int, float, etc.), convert to string
